...
var _createHTTPProxy = require('./createHTTPProxy');

var _createHTTPProxy2 = _interopRequireDefault(_createHTTPProxy);
...
function handleRequest(connection, data, _ref, dstInfo, onConnect, onDestroy, isClientConnected) {
  ...
  clientToRemote.on('data', function (remoteData) {
    if (!decipher) {
      ...
      if (!tmp) {
        _logger2.default.warn(NAME + ' get invalid msg');
        onDestroy();
        return;
      }
...
function createServer(config) {
  ...
  var httpProxyServer = config.enableHTTPProxy ? (0, _createHTTPProxy2.default)(config) : null;
  ...
  return {
    server: server, udpRelay: udpRelay, httpProxyServer: httpProxyServer,
    closeAll: closeAll
  };
}
