...
function createSocksRequest(config, _ref2, cliSocket, chunk) {
  ...
  _socks2.default.createConnection(createSocksOptions(config, host, port), function (err, sslocal) {
    if (err) {
      _logger2.default.warn(NAME + ' get error while create socks request: ' + err.message);
      return;
    }
...
function createHTTPProxy(config) {
  ...
  server.on('connect', function (req, socket, chunk) {
    ...
    _logger2.default.debug(NAME + ' type: CONNECT');
  ...
  server.on('connection', function (socket) {
    ...
    socket.on('error', function (err) {
      _logger2.default.error(NAME + ' socket get error: ' + err.stack);
    });
  });
